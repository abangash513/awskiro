...
### Requesting Pod Identities

At this time, pod identities must be requested from the KPaaS team on an as-needed basis. The process is as follows:

1. **NXOP Team Responsibilities**:
   - Create the target IAM role in the NXOP account with appropriate permissions
   - Define least-privilege policies for specific NXOP resources as required
   - Configure the role's trust relationship to allow assumption from the KPaaS account. This is an example trust policy:
        ...
        "Statement": [
            {
                ...
                "Condition": {
                    "StringEquals": {
                        ...
                        "aws:RequestTag/kubernetes-service-account": "default"
                    },
                    "StringLike": {
   ...
   ```
    ...
    The policy above allows principals in the KPaaS non-prod AWS account (285282426848) to assume the role, assuming the role assumption is originating from an EKS cluster and using the specified namespace and service account.

   - Submit pod identity request through the **NXOP-KPaaS-AWS-Support** Teams channel.
   - Provide the following information to the KPaaS team when requesting:
     - Purpose
     - Namespace
     - Service Account
     - IAM Role ARN in target account (the role to assume)
     - For transparency, provide the trust policy for the target role as well to get a second set of eyes on it and potentially avoid issues that could arise from misconfiguration

1. **KPaaS Team Responsibilities**:
   - Create the intermediate IAM role in the KPaaS account. There are two key components to the role: the trust policy and the actual IAM permissions granted to the role. The trust policy should allow EKS to assume the role. It should look like this:
   ...
   The permissions for the role should allow `sts:AssumeRole` to the role in the NXOP account, like this:
        ...
        "Statement": [
            {
                ...
                "Resource": "arn:aws:iam::178549792225:role/fxip-d-pod-role"
            }
        ]
    }
    ```

   - Create the pod identity configuration linking the Kubernetes service account to the IAM role and specifying the target IAM role in the NXOP account that should be assumed

Both teams should coordinate to ensure proper configuration and testing of the pod identity before deploying workloads that will use the pod identity.
...
- [Runway -- Manage Rancher Project & Namespace](https://developer.aa.com/infrastructure/rancher) --> Landing page for managing resource quota for Rancher Project/Namespace
